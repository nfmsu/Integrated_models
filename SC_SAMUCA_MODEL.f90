subroutine SAMUCA(CONTROL, ISWITCH,                                 &
    ...
    ! MODIFICATION Commented away by JJ 11.4.2024.
    !save
...
40  continue
    ...
    if(flemerged)then
        ...
        select case(method_pop)
            ...
            case(2)       
                ...
                dead_lai=sum(phprof(lf_dpos : (lf_dpos + aint(nsenesleaf_effect)), 5)) *&
		 nstk_now * tilleragefac * 1.e-4 * fdeadlf
	...
	! MODIFICATION IF...THEN.... END IF    
    if((dstr_it_BG + dsug_it_BG) .gt. z .and. supply_used_dw_it_BG .gt. 0.d0)THEN
	 dsug_corr_fac_BG   = supply_used_dw_it_BG/(dstr_it_BG + dsug_it_BG)
	END IF
    if((dstr_it_AG + dsug_it_AG) .gt. z .and. supply_used_dw_it_AG .gt. 0.d0)THEN
	 dsug_corr_fac_AG   = supply_used_dw_it_AG/(dstr_it_AG + dsug_it_AG)
        END IF
